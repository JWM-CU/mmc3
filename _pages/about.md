...
-->
 ...
 {% assign mmc_link = site.data.home-links | where: "name", mmc | first %}
  ...
  <p>The Medicine Microscopy Core (MMC) is a confocal and wide field microscopy core available by reservation through the iLab system. We are part of the <a href="{{ cchd_link.url }}" target="_blank" rel="noopener noreferrer">{{ cchd_link.name }}</a>, the <a href="{{ csct_link.url }}" target="_blank" rel="noopener noreferrer">{{ csct_link.name }}</a> and the <a href="{{ dmed_link.url }}" target="_blank" rel="noopener noreferrer">{{ dmed_link.name }}</a> and provide training, maintenance, and assistance with all instruments.  We specialize in tissue scanning and high-resolution multi-channel fluorescence imaging and welcome collaborations for custom imaging and image analysis.</p>
